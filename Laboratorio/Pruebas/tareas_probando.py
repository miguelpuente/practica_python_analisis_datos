...
class TareaRecurrente(Tarea):
    ...
    def marcar_completa(self):
        """Marks the task as complete if the current date is past the end date."""
        if datetime.now() >= self.fecha_fin:
            super().marcar_completa()

    def to_dict(self):
        """Converts the recurring task to a dictionary format."""
        data = super().to_dict()
        data.update({
...
class TareaProgramada(Tarea):
    ...
    def to_dict(self):
        """Converts the scheduled task to a dictionary format."""
        data = super().to_dict()
        data.update({
...
class GestorDeTareas:
    ...
    def solicitar_fecha(self, mensaje):
        """Solicits a valid date from the user."""
        while True:
    ...
    def seleccionar_tarea(self, tareas):
        """Prompts the user to select a task from a list."""
    ...
    def actualizar_tarea(self, tarea, nuevo_estado):
...
#            return
#        
#        tarea = self.seleccionar_tarea(tareas) if len(tareas) > 1 else tareas[0]
#        
#        try:
#            tarea.estado = nuevo_estado
#            self.guardar_tareas()
